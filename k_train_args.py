...
_EVAL_EVERY = flags.DEFINE_integer("eval_every", 5, "Eval every this many.")
_LOSSES = flags.DEFINE_string(
    "losses",
    "ListMLELoss:1,MSE:0.02",
    'Comma-separated list of "lossName:lossWeight", per `metrics.py`.',
)
_LEARNING_RATE = flags.DEFINE_float("lr", 1e-3, "Learning rate for Adam optimizer.")
...
def get_args() -> TrainArgs:
    return TrainArgs(
        ...
        configs=_NUM_CONFIGS.value,
        early_stop=_EARLY_STOP.value,
        learning_rate=_LEARNING_RATE.value,
        clip_norm=_CLIP_NORM.value,
        model=_MODEL.value,
        model_kwargs_json=_MODEL_KWARGS_JSON.value,
        test_mode=_TEST_MODE.value,
        out_dir=_OUTPUT_DIR.value,
        validate_batches=_VALIDATE_BATCHES.value,
        results_csv=_get_results_csv_or_default(),
