...
_TOY_DATA = False; # Simplified flag usage, as argparse can be used for command-line arguments in PyTorch
_TOY_DATA_VALUE = 100


class TileExample(NamedTuple):
    """Single example of layout graph."""
    total_nodes: torch.Tensor  # shape []
    total_edges: torch.Tensor  # shape []
    total_configs: torch.Tensor  # shape []
    total_config_nodes: torch.Tensor  # shape []

    node_features: torch.Tensor  # shape [total_nodes, node_feat_size]
    node_ops: torch.Tensor  # shape [total_nodes]
    edges: torch.Tensor  # shape [total_edges, 2]
    node_config_features: torch.Tensor  # shape [total_configs, total_config_nodes, conf_feat_size]
    config_runtimes: torch.Tensor  # shape [total_configs]
    argsort_config_runtimes: torch.Tensor  # shape [total_configs]
    graph_id: torch.Tensor  # shape []

    node_config_ids: torch.Tensor  # shape [total_config_nodes]
    node_splits: torch.Tensor

    def to_graph_tensor(self, config_samples: int = -1, max_nodes: int = -1) -> Data:
        """Returns `Data` (sampled if `max(max_nodes, config_samples) >= 0`)."""
        config_features = self.node_config_features
        config_runtimes = self.config_runtimes
        num_config_nodes = config_features.size(1)
        config_node_ids = torch.arange(num_config_nodes)

        if config_samples >= 0:
            input_num_configs = config_runtimes.size(0)
            select_idx = torch.topk(
                ((input_num_configs - torch.arange(input_num_configs)) / input_num_configs).float() -
                torch.log(-torch.log(torch.rand(input_num_configs))),
                config_samples, dim=0)[1]
            
            select_idx = torch.index_select(self.argsort_config_runtimes, 0, select_idx)
            config_runtimes = torch.index_select(config_runtimes, 0, select_idx)
            config_features = torch.index_select(config_features, 0, select_idx)

        full_feed_src = self.edges[:, 0]
        full_feed_tgt = self.edges[:, 1]
        full_config_src = torch.arange(self.node_config_ids.size(0))
        full_config_tgt = self.node_config_ids.int()
        op_node_ids = torch.arange(self.total_nodes)
        node_is_selected = torch.ones([self.total_nodes], dtype=torch.bool)
        kept_node_ratio = torch.tensor(1.0)
        node_ops = self.node_ops
        node_feats = self.node_features

        if max_nodes >= 0:
            num_segments = torch.ceil(self.total_nodes.float() / max_nodes).int()
            segment_id = torch.randint(0, num_segments, (1,)).item()
            start_idx = segment_id * max_nodes
            end_idx = min((segment_id + 1) * max_nodes, self.total_nodes)
            node_is_selected = (op_node_ids >= start_idx) & (op_node_ids < end_idx)

            feed_edge_mask = (self.edges >= start_idx) & (self.edges < end_idx)
            feed_edge_mask = feed_edge_mask[:, 0] & feed_edge_mask[:, 1]
            config_edge_mask = (full_config_tgt >= start_idx) & (full_config_tgt < end_idx)

            kept_node_ratio = ((end_idx - start_idx) / self.total_nodes).float()

            keep_feed_src = full_feed_src[feed_edge_mask]
            keep_feed_tgt = full_feed_tgt[feed_edge_mask]

            keep_config_src = full_config_src[config_edge_mask]
            keep_config_tgt = full_config_tgt[config_edge_mask]

        # Constructing the graph data
        edge_index_feed = torch.stack([full_feed_src, full_feed_tgt], dim=0)
        edge_index_config = torch.stack([full_config_src, full_config_tgt], dim=0)
        edge_index_sampled_feed = torch.stack([keep_feed_src, keep_feed_tgt], dim=0)
        edge_index_sampled_config = torch.stack([keep_config_src, keep_config_tgt], dim=0)
        edge_index_g_op = torch.stack([torch.zeros_like(op_node_ids), op_node_ids], dim=0)
        edge_index_g_config = torch.stack([torch.zeros_like(config_node_ids), config_node_ids], dim=0)

        graph_data = Data(
            x=node_feats,
            edge_index_feed=edge_index_feed,
            edge_index_config=edge_index_config,
            edge_index_sampled_feed=edge_index_sampled_feed,
            edge_index_sampled_config=edge_index_sampled_config,
            edge_index_g_op=edge_index_g_op,
            edge_index_g_config=edge_index_g_config,
            config_features=config_features.transpose(1, 0).transpose(1, 2),
            config_runtimes=config_runtimes,
            kept_node_ratio=kept_node_ratio,
            node_ops=node_ops,
            node_is_selected=node_is_selected
        )
...
class NpzDatasetPartition:
    ...
    def __init__(self):
        ...
        self.node_feat: 'torch.Tensor | None' = None   # indexed by node_ranges.
        self.node_opcode: 'torch.Tensor | None' = None  # ^^
        self.edge_index: 'torch.Tensor | None' = None   # indexed by edge_ranges.
        self.config_feat: 'torch.Tensor | None' = None  # indexed by config_ranges.
    ...
    def load_from_file(self, cache_file: str):
        ...
        self.tile_id = torch.tensor([tile_id.rstrip() for tile_id in tile_ids])
    ...
    def add_npz_file(
        ...
        self._data_dict['tile_id'].append(torch.tensor(np.array(tile_id)))
    ...
    def finalize(self):
        self.tile_id = torch.stack(self._data_dict['tile_id'], dim=0)
        ...
        self.edge_ranges = torch.cumsum(torch.tensor(self._num_edges, dtype=torch.int64), dim=0)
        self.node_ranges = torch.cumsum(torch.tensor(self._num_nodes, dtype=torch.int64), dim=0)
        self.config_ranges = torch.cumsum(torch.tensor(self._num_configs, dtype=torch.int64), dim=0)
...
class NpzDataset(namedtuple('NpzDataset', ['train', 'validation', 'test'])):
    ...
    def _get_normalizer(self, feature_matrix) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def get_npz_split(
  ...
  if cache_dir:
    ...
    filename_hash = hashlib.md5(
        f'{split_path}:{min_configs}:{_TOY_DATA_VALUE}'.encode()).hexdigest()
...
if __name__ == "__main__":
    root_data_path = '/Users/kaiqu/Desktop/kaggle-runtime-optimization/dataset/npz_all/npz/tile/xla'
    get_npz_dataset(root_data_path)
